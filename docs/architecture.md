...
This architecture defines a deterministic, point-in-time safe market data foundation for quantitative research, backtesting, and simulation engines.
...
Binary floating point types (`float`) cannot represent decimal fractions precisely (e.g., $0.1 + 0.2 \ne 0.3$). In quantitative market infrastructure, float errors cause rounding drift, invalid spread calculations, and corrupted execution fills. In this architecture, all canonical prices and quantities are strictly represented using `Decimal` or exact integer-scaled fixed point.
...
Real-world market feeds experience packet jitter and out-of-order delivery. Unbounded in-memory sorting on massive datasets causes $O(N)$ memory exhaustion. The platform implements a bounded priority buffer (min-heap) governed by a configurable lateness watermark:
